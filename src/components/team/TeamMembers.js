...
const TeamMembers = ({ members = [] }) => { // Default empty array to avoid crashes
    ...
    return (
        ...
        >
            {members.map((member) => (
                <Card
                    key={member.playerId || Math.random()} // Fallback key if teamId is missing
                    sx={{
                        display: "flex",
                ...
                >
                    ...
                    <CardContent sx={{ flex: 1, p: 0, textAlign: isSmallScreen ? "center" : "left" }}>
                        ...
                        <Typography variant="body2">
                            <PlaceIcon sx={{ fontSize: "17px" }} /> {member.location}
                        </Typography>

                        {/* Dashed Divider */}
                        <Divider sx={{ my: 1, borderStyle: "dashed" }} />
                        <Typography variant="body2">
                            {member.roleAsBatsman} | {member.roleAsBowler}
                        </Typography>
                    </CardContent>
                </Card>
            ))}
